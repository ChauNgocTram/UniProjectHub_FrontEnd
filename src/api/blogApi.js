...
const getAllBlog = async () => {
  ...
  export const useGetCommentOfBlog = (blogId) => {
    return useQuery(["comment", blogId], () =>
    getCommentOfBlog(blogId)
    );
  };

  //============ add comment
  const createComment = async ({ blogId, description, userId }) => {
    const { data } = await api.post(`${API_ENDPOINTS.COMMENT}/CreateCommentAsync`, {
      blogId,
      description,
      ownerId: userId, 
      status: true, 
...
const deleteComment = async (commentId) => {
    const { data } = await api.delete(`${API_ENDPOINTS.COMMENT}/DeleteCommentAsync/${commentId}`);
    return data;
  };
